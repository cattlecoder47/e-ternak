<div>
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-md-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Referensi</span> - Satuan
                </h4>
    ...
    <div class="content">
        ...
        <div class="card">
            ...
            @include('livewire.referensi.satuan.c_r_satuan')
            @include('livewire.referensi.satuan.u_r_satuan')
            ...
            <div class="table-responsive">
                <table class="table" id="tData">
                    ...
                    <tbody>
                    @foreach($r_satuan as $value)
                        <tr>
                            <td>{{ $value->rsatuan_kode }}</td>
                            <td>{{ $value->rsatuan_nama }}</td>
                            <td width="15%" align="center">
                                <button data-toggle="modal" data-target="#updateModal"
                                        wire:click="edit('{{ $value->rsatuan_kode }}')" class="btn btn-primary btn-sm">
                                    Edit
                                </button>
                                <button wire:click="$emit('triggerDelete','{{ $value->rsatuan_kode }}')"
...
<script type="text/javascript">
    $(function () {
        window.livewire.on('satuanStore', () => {
            $('#createModal').modal('hide');
        });
        window.livewire.on('triggerDelete', rsatuan_kode => {
            bootbox.confirm("APAKAH ANDA AKAN MENGHAPUS SALAH SATU DATA ?", function (result) {
                if (result === true) {
                @this.call('delete', rsatuan_kode)
                }
            });
        });
